...
class BuyPlanSuccess extends React.Component {
  ...
  let url = SERVER_URL+'/orderSuccessful/?orderid='+odnumber
   if(navState.url=='localhost:8000/payUPaymentResponse/'){
     // this.props.navigation.goBack()
     this.props.navigation.navigate('MemberShipSuccess',{odnumber:odnumber,})
   }

   if (navState.url == SERVER_URL+'/orderFailure') {
   ...
   if (navState.url == url) {
     console.log(url,navState.url,'kkkkkkkkkkkkkkkkkk');

    this.props.navigation.navigate('MemberShipSuccess',{
      orderId:odnumber
    })
   }
 };
  ...
  render(){
    ...
    return (
     <View style={{flex:1}}>
      <View style={{height:Constants.statusBarHeight,backgroundColor:'#A5C339'}}></View>
      <View style={{flex:1,}}>
...
export default connect(mapStateToProps, mapDispatchToProps)(BuyPlanSuccess);
