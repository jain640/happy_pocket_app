    ...
    componentDidMount() {
      ...
      this.state.connection.onopen = (session)=>{
        this.setState({session : session});
         ...
         console.log('subscribing to' , WAMP_PREFIX+'ecommerce.chat.' + this.state.myStore.pk);

        session.subscribe(WAMP_PREFIX+'ecommerce.chat.' + this.state.myStore.pk, this.supportChat).then(
        (sub) => {
        },
        (err) => {
...
videoCam = async ()=>{
   ...
   const csrf = await AsyncStorage.getItem('csrf');
    ...
    video.append('sender',this.state.chatWith)
   ...
   _pickImage = async () => {
     ...
     const csrf = await AsyncStorage.getItem('csrf');
       ...
       img.append('sender',this.state.chatWith)
  ...
  postMessage=async(message)=>{
    ...
    formData.append('sender',this.state.chatWith)
      ...
      var themeColor = this.props.store.themeColor
        ...
        <View style={{height:Constants.statusBarHeight,backgroundColor:this.props.store.themeColor}}></View>
          <View style={[styles.shadow,{height:58,alignItems: 'center',flexDirection: 'row',backgroundColor:this.props.store.themeColor}]}>
            ...
            {this.state.getDisplayPic!=null&&
              <Image
                   source={{uri : this.state.displayPic}}
                   style={{ width: 40, height: 40, marginLeft:0 ,marginRight:15,overflow:'hidden',resizeMode:'cover', borderRadius: 40/2, }}
               />
             }
               {this.state.getDisplayPic==null&&
               ...
               <MonoText   style={{color:'#fff',fontSize:20,fontWeight: '400'}} numberOfLines={1}>{this.state.showName}</MonoText>
            </View>
         ...
         renderItem={({item, index})  => {
           ...
           return (
             ...
             {!inMessage&&format=='file'&&
               ...
               {inMessage &&
                  <MonoText   style={styles.time}>{moment(moment.utc(item.created).toDate()).local().format('HH:mm')}</MonoText>
                }
               {!inMessage &&
                  <MonoText   style={styles.timeout}>{moment(moment.utc(item.created).toDate()).local().format('HH:mm')}</MonoText>
              }
               </TouchableOpacity>
             </View>
...
//    backgroundColor:color,
  ...
  export default connect(mapStateToProps, mapDispatchToProps)(ChatUserScreen);
